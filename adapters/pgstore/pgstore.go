...
func sessionRotateSwap(ctx context.Context, db DB, oldKey domain.Token, newSess domain.Session) error {
	res, err := db.ExecContext(ctx, `DELETE FROM authgo_sessions WHERE token = $1`, oldKey.String())
	if err != nil {
		return err
	}
...
func (r *MagicLinkRepo) MarkConsumed(ctx context.Context, hash string) (bool, error) {
	...
	res, err := r.db.ExecContext(ctx, `UPDATE authgo_magic_links SET consumed = TRUE WHERE hash = $1 AND consumed = FALSE`, hash)
	if err != nil {
		return false, err
	}
