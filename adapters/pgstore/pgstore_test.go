...
func TestWorkloadKeyRepo_Integration(t *testing.T) {
	...
	if err := svc.Authorize(raw, "tools:write"); err != nil {
		t.Fatalf("authorize wildcard: %v", err)
	}
	if err := svc.Authorize(raw, "memory:write"); !errors.Is(err, domain.ErrScopeDenied) {
	...
	newKey, newRaw, err := svc.RotateKey(key.ID())
	if err != nil {
		t.Fatalf("rotate: %v", err)
	}
	if _, err := svc.ValidateKey(raw); !errors.Is(err, domain.ErrKeyNotFound) {
		t.Fatalf("old token survived rotate: %v", err)
	}
	if _, err := svc.ValidateKey(newRaw); err != nil {
	...
	if _, err := repo.GetKey(newKey.ID()); !errors.Is(err, domain.ErrNotFound) {
		t.Fatalf("revoke-all missed key: %v", err)
	}

	// Delete unknown → ErrNotFound.
	if err := repo.DeleteKey(domain.KeyID("nope")); !errors.Is(err, domain.ErrNotFound) {
